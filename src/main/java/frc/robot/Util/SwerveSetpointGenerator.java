...
@ExtensionMethod({GeomUtil.class,EpsilonComparison.class})
public class SwerveSetpointGenerator {
  ...
  private double findRoot(
      ...
      double f_1,
      int iterations_left) {
    if (iterations_left < 0 || EpsilonComparison.compare(f_0, f_1)) {
      return 1.0;
    }
  ...
  public SwerveSetpoint generateSetpoint(
      ...
      double dt) {
    final Translation2d[] modules = modulePositions;
    ...
    if (desiredState.toTwist2d().compare(new Twist2d())) {
      need_to_steer = false;
      for (int i = 0; i < modules.length; ++i) {
        desiredModuleState[i].angle = prevSetpoint.states()[i].angle;
    ...
    for (int i = 0; i < modules.length; ++i) {
      prev_vx[i] =
          prevSetpoint.states()[i].angle.getCos()
              * prevSetpoint.states()[i].speedMetersPerSecond;
      prev_vy[i] =
          prevSetpoint.states()[i].angle.getSin()
              * prevSetpoint.states()[i].speedMetersPerSecond;
      prev_heading[i] = prevSetpoint.states()[i].angle;
      if (prevSetpoint.states()[i].speedMetersPerSecond < 0.0) {
    ...
    if (all_modules_should_flip
        && !prevSetpoint.speeds().toTwist2d().compare(new Twist2d())
        && !desiredState.toTwist2d().compare(new Twist2d())) {
    ...
    // limit is exceeded.
    double dx = desiredState.vxMetersPerSecond - prevSetpoint.speeds().vxMetersPerSecond;
    double dy = desiredState.vyMetersPerSecond - prevSetpoint.speeds().vyMetersPerSecond;
    double dtheta =
        desiredState.omegaRadiansPerSecond - prevSetpoint.speeds().omegaRadiansPerSecond;
    ...
    for (int i = 0; i < modules.length; ++i) {
      if (!need_to_steer) {
        overrideSteering.add(Optional.of(prevSetpoint.states()[i].angle));
        continue;
      }
      overrideSteering.add(Optional.empty());
      if (EpsilonComparison.compare(prevSetpoint.states()[i].speedMetersPerSecond, 0.0)) {
        ...
        if (EpsilonComparison.compare(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
          // Goal angle doesn't matter. Just leave module at its current angle.
          overrideSteering.set(i, Optional.of(prevSetpoint.states()[i].angle));
          continue;
        }

        var necessaryRotation =
            prevSetpoint.states()[i].angle.unaryMinus().rotateBy(desiredModuleState[i].angle);
        ...
        if (numStepsNeeded <= 1.0) {
          ...
          overrideSteering.set(
              i,
              Optional.of(
                  prevSetpoint.states()[i].angle.rotateBy(
    ...
    ChassisSpeeds retSpeeds =
        new ChassisSpeeds(
            prevSetpoint.speeds().vxMetersPerSecond + min_s * dx,
            prevSetpoint.speeds().vyMetersPerSecond + min_s * dy,
            prevSetpoint.speeds().omegaRadiansPerSecond + min_s * dtheta);
    ...
    for (int i = 0; i < modules.length; ++i) {
      ...
      final var deltaRotation =
          prevSetpoint.states()[i].angle.unaryMinus().rotateBy(retStates[i].angle);
